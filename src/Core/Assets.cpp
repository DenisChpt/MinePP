...
template<typename T>
Ref<const T> Assets::getFromCache(Cache<T>& cache, const std::string& name, 
...
Ref<const std::string> Assets::loadText(const std::string& path) {
    return getFromCache<std::string>(texts, path, [&path]() {
...
Ref<const Texture> Assets::loadTexture(const std::string& path) {
    return getFromCache<Texture>(textures, path, [this, &path]() {
        return Texture::loadTexture2D(path, *this);
    });
}

Ref<const Texture> Assets::loadTexture2DArray(const std::string& path) {
    return getFromCache<Texture>(textureArrays, path, [this, &path]() {
        return Texture::loadTexture2DArray(path, *this);
    });
}

Ref<const Texture> Assets::loadCubeMap(const std::string& path) {
    return getFromCache<Texture>(cubeMaps, path, [this, &path]() {
        return Texture::loadCubeMapTexture(path, *this);
    });
}

Ref<const Shader> Assets::loadShader(const std::string& path) {
    return getFromCache<Shader>(shaders, path, [this, &path]() {
        return std::make_shared<Shader>(path, *this);
    });
}

Ref<const ShaderProgram> Assets::loadShaderProgram(const std::string& path) {
    return getFromCache<ShaderProgram>(programs, path, [this, &path]() {
        return std::make_shared<ShaderProgram>(path, *this);
    });
}
...
void Assets::TextureAtlas::loadAtlas(const std::string& jsonPath, Assets& assets) {
    ...
    for (const auto& block : j["blocks"]) {
        ...
        if (blockName == "bedrock") blockType = BlockData::BlockType::bedrock;
        else if (blockName == "planks") blockType = BlockData::BlockType::planks;
        else if (blockName == "grass") blockType = BlockData::BlockType::grass;
        else if (blockName == "dirt") blockType = BlockData::BlockType::dirt;
        else if (blockName == "sand") blockType = BlockData::BlockType::sand;
        else if (blockName == "stone") blockType = BlockData::BlockType::stone;
        else if (blockName == "cobblestone") blockType = BlockData::BlockType::cobblestone;
        else if (blockName == "glass") blockType = BlockData::BlockType::glass;
        else if (blockName == "oak_wood") blockType = BlockData::BlockType::oak_wood;
        else if (blockName == "oak_leaves") blockType = BlockData::BlockType::oak_leaves;
        else if (blockName == "water") blockType = BlockData::BlockType::water;
        else if (blockName == "lava") blockType = BlockData::BlockType::lava;
        else if (blockName == "iron") blockType = BlockData::BlockType::iron;
        else if (blockName == "diamond") blockType = BlockData::BlockType::diamond;
        else if (blockName == "gold") blockType = BlockData::BlockType::gold;
        else if (blockName == "obsidian") blockType = BlockData::BlockType::obsidian;
        else if (blockName == "sponge") blockType = BlockData::BlockType::sponge;
        ...
        const std::array<std::string, 6> faceNames = {"top", "east", "west", "north", "south", "bottom"};
        for (int i = 0; i < 6; i++) {
            ...
            if (faces.contains(faceNames[i])) {
                ...
                std::cerr << "No texture found for face " << faceNames[i] << " of block " << blockName << std::endl;
                continue;
            }
    ...
    atlasTexture = std::make_shared<Texture>(GL_TEXTURE_2D_ARRAY, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, true);
    atlasTexture->bind();
    
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA8, tileWidth, tileHeight, 
                 textureData.size(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    
    for (size_t i = 0; i < textureData.size(); i++) {
        glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, tileWidth, tileHeight, 1,
                        GL_RGBA, GL_UNSIGNED_BYTE, textureData[i].data());
    }
