...
Shader::Shader(const std::string& name, Assets& assets)
	: Shader(*assets.loadText(name),
			 name.ends_with(".vert") ? GL_VERTEX_SHADER
									 : (name.ends_with(".frag") ? GL_FRAGMENT_SHADER : 0)) {}
...
ShaderProgram::ShaderProgram(const Ref<const Shader>& vertexShader,
							 const Ref<const Shader>& fragmentShader) {
	TRACE_FUNCTION();
...
void ShaderProgram::setTexture(const std::string& name,
							   const Ref<const Texture>& texture,
							   int32_t slot) const {
	TRACE_FUNCTION();
	setInt(name, slot);
...
Ref<const ShaderProgram> ProceduralShader::getShader() const {
	...
	Ref<const Shader> vertexShader =
		std::make_shared<const Shader>(emitVertexShaderSource(), GL_VERTEX_SHADER);
	Ref<const Shader> fragmentShader =
		std::make_shared<const Shader>(emitFragmentShaderSource(), GL_FRAGMENT_SHADER);

	Ref<const ShaderProgram> shader =
		std::make_shared<const ShaderProgram>(vertexShader, fragmentShader);
	cache[this] = shader;
