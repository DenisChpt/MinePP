...
Framebuffer::Framebuffer(int32_t width,
	...
	if (createDepthAttachment) {
		depthAttachment = std::make_shared<Texture>(
			GL_TEXTURE_2D, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT, GL_FLOAT, false);
		depthAttachment->allocateTexture(width, height);
		glFramebufferTexture2D(
			GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthAttachment->getId(), 0);
	}

	for (uint32_t i = 0; i < colorAttachmentCount; ++i) {
		Ref<Texture> attachment =
			std::make_shared<Texture>(GL_TEXTURE_2D, GL_RGBA16, GL_RGBA, GL_SHORT, false, 0);
		attachment->allocateTexture(width, height);

		uint32_t attachmentName = GL_COLOR_ATTACHMENT0 + i;
		glFramebufferTexture2D(
			GL_FRAMEBUFFER, attachmentName, GL_TEXTURE_2D, attachment->getId(), 0);
