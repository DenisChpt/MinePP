...
inline WorldRayCast::WorldRayCast(glm::vec3 position,
								  glm::vec3 direction,
								  World& world,
								  float reach) {
	successful = false;
	...
	while (!successful && planes[0].getHitDistance() <= reach) {
		...
		if (maybeBlockPosition.has_value() &&
			World::isValidBlockPosition(maybeBlockPosition.value())) {
