...
struct AABB {
	...
	static AABB fromBlockPosition(const glm::vec3& position) {
		return AABB(position, position + glm::vec3{1, 1, 1});
	}
...
class AxisPlane {
	...
	[[nodiscard]] float calculateStartOffset(const glm::vec3& position,
											 const glm::vec3& direction) const;

	[[nodiscard]] float calculateHitDistanceToPosition() const {
		return glm::distance(rayPosition, hitPosition);
	}
	[[nodiscard]] glm::vec3 calculateHitPosition() const;

   public:
	static std::optional<glm::ivec3> rayHitsToBlockPosition(const glm::vec3& hit1,
															const glm::vec3& hit2);
...
inline glm::vec3 AxisPlane::calculateHitPosition() const {
	...
	// if the intersection result is less than 0 then it either did not hit or the hit position is
	// behind the cast position
	if (t < 0)
...
inline float AxisPlane::calculateStartOffset(const glm::vec3& position,
											 const glm::vec3& direction) const {
	return std::floor(glm::dot(planeNormal, position)) +
		   (glm::dot(planeNormal, direction) > 0 ? 1.0f : 0.0f);
}
...
inline std::optional<glm::ivec3> AxisPlane::rayHitsToBlockPosition(const glm::vec3& hit1,
																   const glm::vec3& hit2) {
