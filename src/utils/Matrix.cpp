...
void mat_perspective(float* matrix, float fov, float aspect, float znear, float zfar) {
	float ymax = znear * std::tanf(fov * PI / 360.0f);
...
void set_matrix_2d(float* matrix, int width, int height) {
	mat_ortho(matrix, 0, static_cast<float>(width), 0, static_cast<float>(height), -1, 1);
}
	...
	float fov, int ortho, int radius)
{
	...
	mat_rotate(b, std::cosf(rx), 0, std::sinf(rx), ry);
	mat_multiply(a, b, a);
...
void set_matrix_item(float* matrix, int width, int height, int scale) {
	...
	float xoffset = 1 - size / static_cast<float>(width) * 2;
	float yoffset = 1 - size / static_cast<float>(height) * 2;
